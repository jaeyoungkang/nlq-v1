...
분할된 BigQuery 모듈들을 통합하여 기존 인터페이스 유지
"""
...
class BigQueryClient:
    ...
    def __init__(self, project_id: str, location: str = "asia-northeast3"):
        ...
        self.usage_service = UsageService(project_id, location)
    ...
    def get_conversation_details(self, conversation_id: str, user_id: str = None):
        """특정 대화의 상세 내역 조회"""
    ...
    def get_session_conversation_details(self, conversation_id: str, session_id: str):
        """세션 ID로 특정 대화 상세 조회 (비인증 사용자용)"""
        return self.conversation_service.get_session_conversation_details(conversation_id, session_id)
        
    def link_session_to_user(self, session_id: str, user_id: str, user_email: str):
        """세션 ID의 모든 대화를 사용자 계정으로 연결"""
        return self.conversation_service.link_session_to_user(session_id, user_id, user_email)
    
    # === 사용량 관련 메서드들 ===
    
    def get_usage_count(self, session_id: str, ip_address: str):
        """비인증 사용자의 일일 사용량 조회"""
        return self.usage_service.get_usage_count(session_id, ip_address)
    
    def update_usage_count(self, session_id: str, ip_address: str, user_agent: str = ""):
        """비인증 사용자의 사용량 업데이트"""
        return self.usage_service.update_usage_count(session_id, ip_address, user_agent)
